function [outvar1] = ratertool_eegplot(data, varargin); % p1,p2,p3,p4,p5,p6,p7,p8,p9)
...
if nargin < 1
    help ratertool_eegplot
    return
end

function drawRaterBadChannelMarkers(ax1, g, lowlim, highlim, oldspacing)
try
    delete(findobj(ax1, 'tag', 'raterbadtri'));
catch
end
if isempty(g.raterbadchans)
    return
end
nRaters = numel(g.raterbadchans);
colors = g.ratercolors;
if isempty(colors) || size(colors, 1) < nRaters
    colors = hsv(nRaters);
end
span = max(highlim - lowlim, 1);
% X-axis is window-relative (1..span), so place markers slightly left of 1
xBase = 1 - 0.02 * span;
xStep = 0.015 * span;
offset = (g.dispchans+1) * (oldspacing - g.spacing) / 2 + g.elecoffset * (oldspacing - g.spacing);

for i = 1:g.chans
    chanIdx = g.chans - i + 1;
    y = i * g.spacing + offset;
    for r = 1:nRaters
        idx = g.raterbadchans{r};
        if isempty(idx)
            continue
        end
        if any(idx == chanIdx)
...
function ratertool_eegplot_readkey(src, event)
try
    fig = [];
    try
    ...
    key = '';
    if ~isempty(event) && isfield(event, 'Key') && ~isempty(event.Key)
        key = lower(event.Key);
    elseif ~isempty(event) && isfield(event, 'Character') && ~isempty(event.Character)
        key = event.Character;
    else
        try
    ...
    % Debounce duplicate key events (WindowKeyPressFcn + KeyPressFcn)
    try
        nowSec = now * 86400;
        lastInfo = getappdata(fig, 'ratertool_eegplot_lastkey');
        if isstruct(lastInfo) && isfield(lastInfo, 'key') && isfield(lastInfo, 'time')
            if strcmp(lastInfo.key, key) && (nowSec - lastInfo.time) < 0.15
                return
            end
        end
        setappdata(fig, 'ratertool_eegplot_lastkey', struct('key', key, 'time', nowSec));
    catch
    end

    % Handle arrow keys by character code as fallback
    if isnumeric(key) || (ischar(key) && ~isempty(key) && length(key) == 1)
        if ischar(key)
            code = double(key);
        else
            code = key;
        end
        % 28 = left arrow, 29 = right arrow
        if code == 28
            ratertool_eegplot('drawp', 1); % previous page
            return
        elseif code == 29
            ratertool_eegplot('drawp', 4); % next page
            return
        end
    end
    ...
    switch key
        ...
        case {'rightarrow', 'right'}
            ratertool_eegplot('drawp', 4); % next page
        case 'pageup'
            ratertool_eegplot('drawp', 1);
        case 'pagedown'
            ratertool_eegplot('drawp', 4);
    end
catch
    % ignore keypress errors
end
end
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
    ...
    try
        ...
        disp('ratertool_eegplot() error: calling convention {''key'', value, ... } error'); return;
    end;
    ...
    % ---------------------------------
    defdowncom   = 'ratertool_eegplot(''defdowncom'',   gcbf);'; % push button: create/remove window
    defmotioncom = 'ratertool_eegplot(''defmotioncom'', gcbf);'; % motion button: move windows or display current position
    defupcom     = 'ratertool_eegplot(''defupcom'',     gcbf);';
    defctrldowncom = 'ratertool_eegplot(''topoplot'',   gcbf);'; % CTRL press and motion -> do nothing by default
    ...
    try, g.raterbadchans; 	catch, g.raterbadchans = {}; end;
    try, g.ratercolors; 	catch, g.ratercolors = []; end;
    ...
    try, g.title; 		    catch, g.title		= ['Scroll activity -- ratertool_eegplot()']; 	end;
    ...
    try, g.tag; 				catch, g.tag		= 'ratertool_eegplot'; end;
    ...
    for index=1:length(gfields)
        switch gfields{index}
            case {'spacing', 'srate' 'eloc_file' 'raterbadchans' 'ratercolors' 'winlength' 'position' 'title' 'plottitle' ...
            ...
            otherwise, error(['ratertool_eegplot: unrecognized option: ''' gfields{index} '''' ]);
        end;
    end;
    ...
    if isstr(g.eloc_file) | isstruct(g.eloc_file)  % Read in electrode names
        ...
        ratertool_eegplot('setelect', g.eloc_file, ax1);
    end;
    ...
    u(5) = uicontrol('Parent',figh, ...
        ...
        'Callback', 'ratertool_eegplot(''drawp'',0);' );
    ...
    u(6) = uicontrol('Parent',figh, ...
        ...
        'Callback', 'ratertool_eegplot(''draws'',0);' );
    ...
    u(7) = uicontrol('Parent',figh, ...
        ...
        'string','+',...
        'FontSize',8,...
        'Callback','ratertool_eegplot(''draws'',1)');
    u(8) = uicontrol('Parent',figh, ...
        ...
        'string','-',...
        'FontSize',8,...
        'Callback','ratertool_eegplot(''draws'',2)');
    ...
    cb_envelope = ['g = get(gcbf,''userdata'');'...
        ...
        'ratertool_eegplot(''drawp'',0);clear g;'];
    ...
    if ~isempty(g.events)
        u(17) = uicontrol('Parent',figh, ...
            'Units', 'normalized', ...
            'Position',posbut(17,:), ...
            'string', 'Event types', 'callback', 'ratertool_eegplot(''drawlegend'', gcbf)');
    end;
    ...
    uimenu('Parent',m(7),'Label','Edit figure', 'Callback', 'ratertool_eegplot(''noui'');');
    ...
    uimenu('Parent',m(11),'Label','Hide marks','Callback', ...
        ...
        'set(gcbf, ''userdata'', g);' ...
        'ratertool_eegplot(''drawb''); clear g;'] )
    ...
    % Scale Eye %%%%%%%%%
    timestring = ['[OBJ1,FIG1] = gcbo;',...
        'ratertool_eegplot(''scaleeye'',OBJ1,FIG1);',...
        'clear OBJ1 FIG1;'];
    m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);
    
    % Title %%%%%%%%%%%%
    uimenu('Parent',m(1),'Label','Title','Callback','ratertool_eegplot(''title'')')
    ...
    uimenu('Parent',m(2),'Label','Time range to display',...
        'Callback','ratertool_eegplot(''window'')')
    
    % Electrode window %%%%%%%%
    uimenu('Parent',m(2),'Label','Number of channels to display',...
        'Callback','ratertool_eegplot(''winelec'')')
    ...
    uimenu('Parent',m(6),'Label','Load .loc(s) file',...
        'Callback','ratertool_eegplot(''loadelect'');')
    
    % Zooms %%%%%%%%
    zm = uimenu('Parent',m(2),'Label','Zoom off/on');
    if verLessThan('matlab','8.4.0')
        commandzoom = [ 'set(gcbf, ''WindowButtonDownFcn'', [ ''zoom(gcbf,''''down''''); ratertool_eegplot(''''zoom'''', gcbf, 1);'' ]);' ...
        ...
        commandzoom = [ 'wtemp = warning; warning off;set(gcbf, ''WindowButtonDownFcn'', [ ''zoom(gcbf); ratertool_eegplot(''''zoom'''', gcbf, 1);'' ]);' ...
            'tmpg = get(gcbf, ''userdata'');' ...
            'warning(wtemp);'...
    ...
    %   uimenu('Parent',figh,'Label', 'Help', 'callback', 'pophelp(''ratertool_eegplot'');');
    
    % Events %%%%%%%%
    zm = uimenu('Parent',m(2),'Label','Events');
    complotevent = [ 'tmpg = get(gcbf, ''userdata'');' ...
        'tmpg.plotevent = ''on'';' ...
        'set(gcbf, ''userdata'', tmpg); clear tmpg; ratertool_eegplot(''drawp'', 0);'];
    comnoevent   = [ 'tmpg = get(gcbf, ''userdata'');' ...
        'tmpg.plotevent = ''off'';' ...
        'set(gcbf, ''userdata'', tmpg); clear tmpg; ratertool_eegplot(''drawp'', 0);'];
    comeventmaxstring   = [ 'tmpg = get(gcbf, ''userdata'');' ...
        'tmpg.plotevent = ''on'';' ...
        'set(gcbf, ''userdata'', tmpg); clear tmpg; ratertool_eegplot(''emaxstring'');']; % JavierLC
    comeventleg  = [ 'ratertool_eegplot(''drawlegend'', gcbf);'];
    ...
    ratertool_eegplot('scaleeye', [], gcf);
    if strcmp(lower(g.scale), 'off')
        ratertool_eegplot('scaleeye', 'off', gcf);
    end;
    
    ratertool_eegplot('drawp', 0);
    ratertool_eegplot('drawp', 0);
    if g.dispchans ~= g.chans
        ratertool_eegplot('zoom', gcf);
    end;
    ratertool_eegplot('scaleeye', [], gcf);
    ...
    switch data
        case 'drawp' % Redraw EEG and change position
            ...
            % this test help to couple ratertool_eegplot windows
            if exist('p3', 'var')
                figh = p3;
                figure(p3);
            else
            ...
            % channels in the ratertool_eegplot figure window.
            for i = 1:g.chans
            ...
            % draw per-rater bad-channel markers (triangles on left of labels)
            if isfield(g, 'raterbadchans') && ~isempty(g.raterbadchans)
                drawRaterBadChannelMarkers(ax1, g, lowlim, highlim, oldspacing);
            end
            ...
            if g.children ~= 0
                ...
                ratertool_eegplot( 'drawp', p1, p2, g.children);
                figure(figh);
            end;
        ...
        case 'winelec'  % change channel window size
            ...
            set(gcf, 'UserData', g);
            ratertool_eegplot('updateslider', fig);
            ratertool_eegplot('drawp',0);
            ratertool_eegplot('scaleeye', [], fig);
            return;
        ...
        case 'loadelect' % load channels
            ...
            ratertool_eegplot('setelect',[ inputpath inputname ],AXH0);
            return;
        ...
        case 'noui'
            if ~isempty(varargin)
                ratertool_eegplot( varargin{:} ); fig = gcf;
            else
                fig = findobj('tag', 'ratertool_eegplot');
            end;
        ...
        case 'zoom' % if zoom
            ...
            if exist('p2', 'var') == 1
                if verLessThan('matlab','8.4.0')
                    set(gcbf, 'windowbuttondownfcn', [ 'zoom(gcbf,''down''); ratertool_eegplot(''zoom'', gcbf, 1);' ]);
                else
                    % This is failing for us: http://undocumentedmatlab.com/blog/enabling-user-callbacks-during-zoom-pan
                    ...
                    set(gcbf, 'WindowButtonDownFcn', [ 'zoom(gcbf); ratertool_eegplot(''zoom'', gcbf, 1);' ]);
                    warning(wtemp);
                end
            end;
            ...
            if strcmp(get(fig, 'SelectionType'),'normal');
                ...
                if g.incallback ~= 1 % interception of nestest calls
                    ...
                    if (tmppos(1) >= 0) & (tmppos(1) <= highlim),
                        ...
                        set(fig,'UserData', g);
                        ratertool_eegplot('drawp', 0);  % redraw background
                    end;
                end;
        ...
        otherwise
            error(['Error - invalid ratertool_eegplot() parameter: ',data])
    end
end
...
if ~exist('fig','var')
    ...
    cmd5='ratertool_eegplot( EEG.data(EEG.icachansind,:,:), ''srate'', EEG.srate, ''title'', ''Black = channel before rejection; red = after rejection -- ratertool_eegplot()'',''limits'', [EEG.xmin EEG.xmax]*1000, ''data2'', compproj);';
	...
	cmd_ic_activiation2='ratertool_eegplot( tmpdata, ''srate'', EEG.srate, ''title'', ''Scroll component activities -- ratertool_eegplot()'',''limits'', [EEG.xmin EEG.xmax]*1000);'
